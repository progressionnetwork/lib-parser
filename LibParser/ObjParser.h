...
protected:
	void GetNameofSymb(PIMAGE_SYMBOL pSymbol,FuncHeader& funcHead, CHAR* m_NameFileName);
	void GetDataofSymb(PIMAGE_SYMBOL pSymbol,FuncHeader& funcHead);

	void MarkRelocatePos(PIMAGE_SECTION_HEADER pISH);
public:
	BOOL Parse(PBYTE objImage,FILE* pNamFile,FILE* pDatFile,FuncHeaderTable* funcTable, CHAR* m_NameFileName);
